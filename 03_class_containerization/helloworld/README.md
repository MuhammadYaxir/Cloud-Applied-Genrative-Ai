...
docker ps -a
```
**Come out from Container without stopping Container**
...
```bash
docker exec -it Conatiner Id first four digits /bin/bash
```
**Intract with the Container if bash is not available then sh:**

```bash
docker exec -it dev-cont1 /bin/sh
```
...
```
CMD can be override but ENTRYPOINT can't be override. If we manually instll dependencies in container then CMD will not run
```bash
